...
class _DoctorDetailsScreenState extends State<DoctorDetailsScreen> {
  ...
  _buildBottomButtons(BuildContext context, DocProvider provider) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: SElevatedButton(
              onPressed: provider.isSaving ? null : () => _saveToDb(provider),
              child: Text(
                "save".tr(context),
              ),
            ),
  ...
  _buildAppBar(BuildContext context, DocProvider provider, double avatarSize) {
    return SliverAppBar(
      pinned: true,
                            ...
                            itemBuilder: (BuildContext context) {
                              return [
                                PopupMenuItem(
                                  ...
                                  child: Row(
                                    children: [
                                      const Icon(Remix.camera_2_line),
                                      const SizedBox(width: 5),
  ...
  _buildBody(BuildContext context, DocProvider provider, double factor) {
  ...
  _pickImage(DocProvider provider, ImageSource source) {
    ImagePicker.platform
        .getImage(source: source, maxHeight: 1200, imageQuality: 60)
        .then((value) {
      if (value is XFile) {
        _updated = true;
        provider.updatePic(_doctor, value.path);
        _updated = true;
      }
    }).catchError((err) {});
  }
